...
6. 树莓派通过docker安装openwrt方法
   ...
   参考教程[在Docker 中运行 OpenWrt 旁路网关](https://mlapp.cn/376.html)
    ...
    ubuntu20.04 可以通过service服务的方式自动开启macvlan, 参考[askubuntu中的回答](https://askubuntu.com/a/1356228/1386748)
...
7. frpc 设置为系统服务
    ...
    下载frp软件, 将`systemd`, 修改配置 `/etc/systemd/system`, 开启
    `systemctl start frpc.service`, 设置开机启动`sudo systemctl enable frpc.service`  

8. centos, ubuntu, pipy 等替换阿里源
