...
- [备忘学习文档study](#备忘学习文档study)
  - [idea](#idea)
    - [maven 控制台中文乱码](#maven-控制台中文乱码)
    - [正常启动控制台乱码](#正常启动控制台乱码)
    - [快捷键 shortcuts](#快捷键-shortcuts)
    - [自定义快捷键](#自定义快捷键)
    - [常用技巧](#常用技巧)
    - [文件颜色](#文件颜色)
    - [idea build artifacts 二进制文件乱码](#idea-build-artifacts-二进制文件乱码)
    - [idea 取消自动更新](#idea-取消自动更新)
    - [编码快捷](#编码快捷)
    - [发布模式区别](#发布模式区别)
    - [在intellij idea中打jar包, 运行提示没有主清单属性](#在intellij-idea中打jar包-运行提示没有主清单属性)
    - [Cannot resolve symbol 'SpringApplication'](#cannot-resolve-symbol-springapplication)
    - [远程debug](#远程debug)
    - [Git Pull Failed](#git-pull-failed)
    - [maven设置](#maven设置)
      - [配置aliyun仓库地址](#配置aliyun仓库地址)
      - [maven乱码](#maven乱码)
      - [maven 测试 OutOfMemoryError](#maven-测试-outofmemoryerror)
      - [maven 无法下载源码 cannot download sources](#maven-无法下载源码-cannot-download-sources)
      - [maven 测试单个类](#maven-测试单个类)
    - [intellij 出现"Usage of API documented as @since 1.6+"的解决办法](#intellij-出现usage-of-api-documented-as-since-16的解决办法)
    - [ctrl+space冲突问题](#ctrlspace冲突问题)
    - [修改提示首字母区分大小写的问题](#修改提示首字母区分大小写的问题)
    - [日志](#日志)
    - [Intellij IDEA 自动生成 serialVersionUID](#intellij-idea-自动生成-serialversionuid)
    - [查看javadoc](#查看javadoc)
    - [忽略git提交文件](#忽略git提交文件)
    - [SVN版本管理](#svn版本管理)
    - [lombok 找不到符号](#lombok-找不到符号)
    - [最大化控制台 maximize console window](#最大化控制台-maximize-console-window)
    - [临时作为文本编辑器 scratch file](#临时作为文本编辑器-scratch-file)
    - [找回spring boot `run dashboard` 面板](#找回spring-boot-run-dashboard-面板)
  - [tomcat乱码](#tomcat乱码)
  - [chrome浏览器](#chrome浏览器)
    - [常用快捷键hotkey](#常用快捷键hotkey)
    - [插件离线下载地址](#插件离线下载地址)
    - [ajax访问file协议跨域](#ajax访问file协议跨域)
    - [查看事件监听listener](#查看事件监听listener)
    - [调试模式找不到ajax请求的js](#调试模式找不到ajax请求的js)
    - [模拟网速慢的情况low net speed](#模拟网速慢的情况low-net-speed)
    - [小于12px的字体不渲染的解决方案](#小于12px的字体不渲染的解决方案)
    - [添加受信任的https网站](#添加受信任的https网站)
    - [ajax loading图](#ajax-loading图)
    - [ajax 序列化数组参数](#ajax-序列化数组参数)
    - [关闭download bar](#关闭download-bar)
    - [google搜索技巧](#google搜索技巧)
  - [http](#http)
    - [跨域 CORS](#跨域-cors)
    - [浏览器缓存机制cache](#浏览器缓存机制cache)
    - [postman 请求问题](#postman-请求问题)
    - [XSS 和 CSRF](#xss-和-csrf)
      - [XSS](#xss)
      - [CSRF](#csrf)
  - [html](#html)
    - [a标签click不触发转跳](#a标签click不触发转跳)
    - [a标签热键](#a标签热键)
    - [事件阶段event](#事件阶段event)
  - [javascript](#javascript)
    - [三种声明方式的区别decare](#三种声明方式的区别decare)
    - [undefinded](#undefinded)
    - [js中的this](#js中的this)
    - [jquery动态事件绑定](#jquery动态事件绑定)
    - [JS打开新窗口的2种方式](#js打开新窗口的2种方式)
    - [javascript 函数call apply区别](#javascript-函数call-apply区别)
    - [javascript 正则表达式可视化](#javascript-正则表达式可视化)
      - [正则表达式  regex (regular expressions)](#正则表达式--regex-regular-expressions)
    - [bootstrap modal 窗口闪现消失的问题](#bootstrap-modal-窗口闪现消失的问题)
    - [javascript const用法](#javascript-const用法)
    - [tampermonkey 引用本地编辑器的方法](#tampermonkey-引用本地编辑器的方法)
    - [字节序-大小端 endian](#字节序-大小端-endian)
    - [字符串转unicode码](#字符串转unicode码)
      - [vue学习](#vue学习)
        - [v-if vs v-show](#v-if-vs-v-show)
  - [nodejs](#nodejs)
    - [淘宝源](#淘宝源)
    - [npm相关设置](#npm相关设置)
    - [npx 运行http-server](#npx-运行http-server)
    - [使用添加ssl证书](#使用添加ssl证书)
      - [证书格式](#证书格式)
    - [vscode 开发 typescript项目](#vscode-开发-typescript项目)
    - [异步函数执行顺序](#异步函数执行顺序)
    - [npm install](#npm-install)
  - [mongo](#mongo)
    - [CAP理论](#cap理论)
    - [安装步骤install](#安装步骤install)
    - [备份与恢复 backup and restore](#备份与恢复-backup-and-restore)
  - [freemarker 语法](#freemarker-语法)
  - [专业名词](#专业名词)
    - [IaaS PaaS Saas 区别](#iaas-paas-saas-区别)
  - [JAVA](#java)
    - [API/SPI区别](#apispi区别)
    - [枚举类通过值获取对象](#枚举类通过值获取对象)
    - [logback配置文件中additivity的作用](#logback配置文件中additivity的作用)
    - [ocr识别(图片转文字)](#ocr识别图片转文字)
    - [线程 thread](#线程-thread)
    - [io 操作](#io-操作)
      - [bio nio aio区别](#bio-nio-aio区别)
      - [io操作分类](#io操作分类)
    - [引用 reference](#引用-reference)
    - [finally和return执行顺序](#finally和return执行顺序)
    - [java中的常见错误](#java中的常见错误)
    - [登录权限spring security](#登录权限spring-security)
    - [POI (Poor Obfuscation Implementation) 可怜的模糊实现](#poi-poor-obfuscation-implementation-可怜的模糊实现)
    - [jackson 时间序列化问题](#jackson-时间序列化问题)
    - [单元测试](#单元测试)
      - [JMockit 在IDEA中的用法](#jmockit-在idea中的用法)
    - [代码片段](#代码片段)
      - [array转字符串拼接](#array转字符串拼接)
    - [框架设计](#框架设计)
      - [GSON](#gson)
    - [gradle](#gradle)
      - [gradle 仓库使用代理](#gradle-仓库使用代理)
  - [git命令](#git命令)
    - [解决git pull push每次都需要输入密码问题](#解决git-pull-push每次都需要输入密码问题)
    - [git使用ssh方式连接github](#git使用ssh方式连接github)
    - [gitignore文件用法](#gitignore文件用法)
    - [查看命令语法 git help](#查看命令语法-git-help)
    - [日志命令 git log](#日志命令-git-log)
    - [git remote 由于远程分支删除导致本地与远程不一致问题error: cannot lock ref](#git-remote-由于远程分支删除导致本地与远程不一致问题error-cannot-lock-ref)
    - [git 设置代理](#git-设置代理)
    - [git windows乱码](#git-windows乱码)
    - [git 行尾行尾crlf换行符\\n与\\r\\n处理, 文件属性](#git-行尾行尾crlf换行符n与rn处理-文件属性)
    - [git windows文件名过长](#git-windows文件名过长)
    - [git 关联远程(裸仓库)](#git-关联远程裸仓库)
    - [git多用户区分公司/个人](#git多用户区分公司个人)
  - [markdown](#markdown)
    - [code 语法](#code-语法)
    - [转ppt方案](#转ppt方案)
    - [在线转表格 Tables](#在线转表格-tables)
    - [vscode markdownlint error settings](#vscode-markdownlint-error-settings)
  - [mysql](#mysql)
    - [windows 绿色安装](#windows-绿色安装)
    - [绿色安装mysql5.7](#绿色安装mysql57)
    - [window开启mysql服务](#window开启mysql服务)
    - [mysql常用命令](#mysql常用命令)
    - [mysql 复制表结构](#mysql-复制表结构)
    - [mysql 主从复制](#mysql-主从复制)
    - [mysql 创建用户与授权](#mysql-创建用户与授权)
    - [mysql优化](#mysql优化)
    - [字符串拼接与截取](#字符串拼接与截取)
    - [emoji插入失败](#emoji插入失败)
    - [动态拼接表名](#动态拼接表名)
  - [oracle](#oracle)
    - [oracle存储过程 procedure =\>](#oracle存储过程-procedure-)
  - [lucene](#lucene)
  - [maven](#maven)
    - [查询plugin命令](#查询plugin命令)
    - [跳过单元测试](#跳过单元测试)
  - [xshell](#xshell)
    - [xshell 乱码问题](#xshell-乱码问题)
    - [shell 脚本](#shell-脚本)
      - [shell 语法](#shell-语法)
      - [vim命令](#vim命令)
  - [jenkins](#jenkins)
    - [ant启动报错 'ant.bat' 不是内部或外部命令, 也不是可运行的程序](#ant启动报错-antbat-不是内部或外部命令-也不是可运行的程序)
  - [sonar](#sonar)
    - [sonar各级别代表的意思](#sonar各级别代表的意思)
  - [SpringMVC](#springmvc)
  - [Spring](#spring)
    - [validate校验](#validate校验)
    - [源码分析](#源码分析)
      - [IoC](#ioc)
      - [AOP](#aop)
    - [spring-boot](#spring-boot)
      - [spring boot starter 和 spring boot auto-configuration 原理](#spring-boot-starter-和-spring-boot-auto-configuration-原理)
      - [spring-boot 启动方式](#spring-boot-启动方式)
      - [spring start initializr 本地搭建](#spring-start-initializr-本地搭建)
  - [autohotkey](#autohotkey)
    - [变量](#变量)
    - [autohotkey自启动](#autohotkey自启动)
    - [Win10操作系统找不到已输入的环境选项解决方案](#win10操作系统找不到已输入的环境选项解决方案)
  - [Total Commander](#total-commander)
    - [设置关闭按钮最小化tc minimize](#设置关闭按钮最小化tc-minimize)
    - [快捷键 tc shortcuts](#快捷键-tc-shortcuts)
  - [nginx](#nginx)
    - [413 Request Entity Too Large的解决方法](#413-request-entity-too-large的解决方法)
  - [运维相关](#运维相关)
    - [docker](#docker)
      - [windows 安装](#windows-安装)
      - [docker toolbox 使用宿主主机代理](#docker-toolbox-使用宿主主机代理)
      - [docker 配置国内镜像地址](#docker-配置国内镜像地址)
      - [docker proxy](#docker-proxy)
      - [docker wsl2位置迁移](#docker-wsl2位置迁移)
    - [wsl2 迁移ubuntu 设置默认用户](#wsl2-迁移ubuntu-设置默认用户)
      - [docker 导出所有镜像](#docker-导出所有镜像)
      - [wsl迁移的其他方法](#wsl迁移的其他方法)
      - [wsl2设置资源限制](#wsl2设置资源限制)
      - [wsl2的centos7开启sshd服务](#wsl2的centos7开启sshd服务)
      - [docker 与 vmware共存](#docker-与-vmware共存)
      - [docker中CMD与ENTRYPOTINT区别](#docker中cmd与entrypotint区别)
      - [docker修改restart参数](#docker修改restart参数)
      - [docker查看启动命令](#docker查看启动命令)
      - [vmware 共享文件夹不可用](#vmware-共享文件夹不可用)
      - [vmware ubuntu 网络不可用](#vmware-ubuntu-网络不可用)
      - [docker安装mysql](#docker安装mysql)
      - [docker安装redis](#docker安装redis)
    - [k8s](#k8s)
      - [k8s中使用wsl2的路径](#k8s中使用wsl2的路径)
    - [rancher](#rancher)
  - [office](#office)
    - [VBA数组下标可以是负数](#vba数组下标可以是负数)
    - [excel中vlookup函数](#excel中vlookup函数)
  - [windows](#windows)
    - [path环境变量命令](#path环境变量命令)
    - [bat 用法](#bat-用法)
      - [getDesktopPath bat](#getdesktoppath-bat)
      - [bat changePath](#bat-changepath)
      - [cmd中复制粘贴](#cmd中复制粘贴)
      - [bat更换壁纸](#bat更换壁纸)
      - [bat 不显示小黑框](#bat-不显示小黑框)
      - [bat 中的路径](#bat-中的路径)
      - [bat vbs其他技巧](#bat-vbs其他技巧)
      - [telnet 退出命令](#telnet-退出命令)
      - [bat打包制定文件到jar包中](#bat打包制定文件到jar包中)
      - [重新获取ip地址](#重新获取ip地址)
      - [cmd没有颜色,重置cmd设置](#cmd没有颜色重置cmd设置)
      - [cmd编码](#cmd编码)
      - [cmd别名doskey, 如定义别名返回上级目录](#cmd别名doskey-如定义别名返回上级目录)
    - [Win7不按Shift, 直接右键显示"在此处打开命令窗口(W)"](#win7不按shift-直接右键显示在此处打开命令窗口w)
    - [win10 multi desktop多桌面快捷键](#win10-multi-desktop多桌面快捷键)
    - [windows service 查找服务名](#windows-service-查找服务名)
      - [Linux命令补齐](#linux命令补齐)
    - [windows win+r cmd 常用命令](#windows-winr-cmd-常用命令)
    - [windows清除网上邻居帐号](#windows清除网上邻居帐号)
    - [windows kill 进程命令](#windows-kill-进程命令)
    - [windows 隐藏盘符](#windows-隐藏盘符)
    - [windows添加管理单元 如管理受信证书](#windows添加管理单元-如管理受信证书)
    - [windows 显示密码](#windows-显示密码)
    - [真正的管理员 administrator](#真正的管理员-administrator)
    - [vscode用法](#vscode用法)
    - [windows 远程连接linux](#windows-远程连接linux)
      - [离线安装插件脚本](#离线安装插件脚本)
    - [图片与压缩包合并](#图片与压缩包合并)
    - [系统变量 environment](#系统变量-environment)
    - [windows right click 修改右键位置](#windows-right-click-修改右键位置)
    - [windows 几种链接机制](#windows-几种链接机制)
    - [windows 查看开关机时间与重启原因](#windows-查看开关机时间与重启原因)
      - [安装windows10 工具](#安装windows10-工具)
      - [windows10 使用wsl2 安装 centos等](#windows10-使用wsl2-安装-centos等)
      - [win10查看电池使用情况](#win10查看电池使用情况)
    - [vscode 快捷操作](#vscode-快捷操作)
    - [cmder 用法](#cmder-用法)
      - [cmder升级](#cmder升级)
      - [cmder问题](#cmder问题)
      - [cmder 修改命令提示符](#cmder-修改命令提示符)
      - [cmder 整合进vscode](#cmder-整合进vscode)
      - [cmder 整合 intellij idea](#cmder-整合-intellij-idea)
      - [cmder 注册右键打开](#cmder-注册右键打开)
    - [代理问题](#代理问题)
      - [socks5转http代理](#socks5转http代理)
    - [jmeter](#jmeter)
      - [jmeter 不能双击打开](#jmeter-不能双击打开)
      - [jmeter 压测端口占用问题](#jmeter-压测端口占用问题)
...
1. 需要在远程服务器上开启调试模式, jdk1.5以前`-Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=5005`, jdk1.5以后`-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5005`, spring boot 的 maven项目需要通过`mvn spring-boot:run -Drun.jvmArguments="-Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=5005"`添加, 或者通过插件的形式配置jvm的启动参数  
2. 在本地, 添加remote的配置, mode选attach to remote jvm, transport 选 socket, 设置host和port, 并配置use module classpath,  然后debug启动即可  
3. 如果本地项目是直接通过添加jvm参数启动的, 可以用 attach debugger 直接连接到本地的服务即可, 或者通过`attach to process`命令连接到对应的服务  
4. ps:远程服务器一般不会开测试的端口, 也不允许远程访问, 所以一般这个功能没有什么用, 可以作为本地临时进行debug测试的补充. 其中suspend=y, 为等待调试着连接之后再执行, suspend=n则不等, 一般调试启动过程中的时候, 可以开启此选项, 否则无法调试启动时的错误  
...
## git协议走代理  
git config core.gitproxy socks5://127.0.0.1:3658  
```  

github的ssh协议走代理, 参考[ssh协议走代理](https://cms-sw.github.io/tutorial-proxy.html)  
...
参考[记录一下Windows 10下安装MySQL 5.7.33](https://blog.csdn.net/zhangbeizhen18/article/details/116035930)
...
1. 添加配置文件, `my.ini`
    ...
    port=3306
    #设置mysql安装目录
    basedir=C:\LS\mysql-5.7.33-winx64
    #设置数据库数据存放目录
    datadir=C:\LS\mysql-5.7.33-winx64\data
    #允许最大连接数
    max_connections=180
    #服务端字符集
    character-set-server=utf8
...
1. 配置系统变量`MYSQL_HOME`值`C:\LS\mysql-5.7.33-winx64`, `PATH`追加`%MYSQL_HOME%\bin`, 或执行以下命令

```bat
setx MYSQL_HOME "C:\LS\mysql-5.7.33-winx64"
for /f "usebackq tokens=2,*" %A in (`reg query HKCU\Environment /v PATH`) do set my_user_path=%B
setx PATH %my_user_path%;^%MYSQL_HOME^%\bin
rem % in bat
rem setx PATH %my_user_path%;%%MYSQL_HOME%%\bin
rem setx PATH "%my_user_path%;%%MYSQL_HOME%%\bin"
```

1. 进入安装bin目录执行`mysqld install`

1. 之后执行`mysqld --initialize --console`, 查看输出的密码

1. `net start mysql`启动服务

1. `mysql -u root -p`进入mysql客户端, 输入刚才输出的密码

1. `ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY '你的密码';` 修改密码
...
`F4` 打开文件, 可在 配置->操作方式->编辑/查看中的编辑器选项中进行修改, 添加vscode, `"C:\Users\Lenovo\AppData\Local\Programs\Microsoft VS Code\Code.exe" "%1"`  
新建命令参数, 参考 [Total Commander Button Bar Parameters](https://www.cheatography.com/deejroth/cheat-sheets/total-commander-button-bar-parameters/) 其中`%N`是当前文件名  
以压缩包形式显示`jar`, 在`Wincmd.ini`文件中的`PackerPlugins`, 复制一行和7z配置一样即可  
...
- client 代理 [proxy](https://docs.docker.com/network/proxy/)  

在客户端17.07以上`~/.docker/config.json`文件  
...
- server代理 [server代理](https://docs.docker.com/config/daemon/systemd/)  
...
[windows-docker-desktop-代理](https://docs.docker.com/docker-for-windows/#proxies)  
...
### wsl2 迁移ubuntu 设置默认用户

进入ubuntu, create/edit `/etc/wsl.conf`

查看[How to change default user in WSL Ubuntu bash on Windows 10](https://askubuntu.com/questions/816732/how-to-change-default-user-in-wsl-ubuntu-bash-on-windows-10)
...
- 关闭正在运行的虚拟机 `wsl --shutdown`  
- 找到 `计算机\HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Lxss\`  
- 找到对应子项的 `BasePath`, 复制ext4.vhdx到新位置, (修改文件夹权限,如果出现拒绝访问. )  
- 修改`BasePath`  

#### wsl2设置资源限制  

```conf  
# 创建%userprofile%\.wslconfig, 设置一下属性  
[wsl2]  
memory=2GB  
processors=4  
swap=512MB  
...
#### wsl2的centos7开启sshd服务  

```shell  
yum install openssh-server  
# change /etc/ssh/sshd_config, Port and so on  
# systemctl start ssh  
# 上述命令没有反应,可使用  
sudo mkdir -p /run/sshd; sudo /usr/sbin/sshd -f /etc/ssh/sshd_config  
# 前端执行  
# sudo mkdir -p /run/sshd; sudo /usr/sbin/sshd -f /etc/ssh/sshd_config -d -D  
```  
...
覆盖`cmd`和`entrypoint`的方法

- `cmd`: docker run 中最后的参数
- `entrypoint`: 命令行指定`-entrypoint`参数
...
参考[linuxea:如何复现查看docker run参数命令](http://www.linuxea.com/2270.html)
...
参考 [VMware Workstation Ubuntu 20.04 LTS无法连接网络问题](https://blog.csdn.net/qq_36393978/article/details/106472278)
...
如果`/etc/NetworkManager/NetworkManager.conf`文件不存在
...
linux下因为logs权限问题启动不起来, 或者直接删掉`logs`目录, 参考[docker安装redis](https://blog.lautumn.cn/docker/2021/09/03/docker-compose-mysql-redis.html#docker%E5%AE%89%E8%A3%85redis)
...
```shell  
# 如果要映射到主机上的话路径前加 /run/desktop/mnt/host  
...
```bat  
    ...
    for /f "usebackq tokens=2,*" %A in (`reg query HKCU\Environment /v PATH`) do set my_user_path=%B
    setx PATH "%my_user_path%;D:\Program Files\"  

    REM 使用setx设置为永久环境变量,适用于bat中:(系统)--有问题%path%会是当前用户和系统汇总之后的  
    ...
    setx PATH "%SYSPATH%;D:\Program Files\"  /M
...
`%UserProfile%` 局部 返回当前用户的配置文件的位置, 当前用户家目录.  
...
- [`msdn itellyou`](https://next.itellyou.cn/)  

#### windows10 使用wsl2 安装 centos等  

[参考相关文档](https://wsldl-pg.github.io/docs/Using-wsldl/)  
...
参考 [cmder升级](https://github.com/cmderdev/cmder#upgrading)

1. 【可选】退出所有 Cmder 会话并重新启动 cmder.exe，这会将你现有的[cmder_root]/vendor/conemu-maximus5/conemu.xml 备份到[cmder_root]/config/user[-|_]conemu.xml.（该文件包含你使用 "Setup Tasks" 设置对话框所做的任何自定义设置）
1. 【可选】退出所有 Cmder 会话并备份你在[Cmder root]/vendor 下手动编辑的所有文件。（所以说一般不建议修改这下面的文件，所有用户自定义都应该在 [cmder_root]/config 文件夹）
1. 删除 [cmder_root]/vendor 文件夹
1. 解压 cmder 压缩包到 [cmder_root]/ 目录，并选择覆盖所有文件。

#### cmder问题  

- 汉字重叠   `Setting->Main` 取消 `Compress long strings to fit space`的勾选  
- 当前路径新建窗口, 新建宏 `Shell("new_console:I")`快捷键 `ctrl+n`, `Shell("new_console:Is50")`快捷键 `ctrl+shift+n`, `Shell("new_console:aI")`快捷键`ctrl+alt+n`管理员权限, 有时需要`Shell("new_console:aI","","","%CD%")`  
...
```lua  
-- 单纯复制函数修改, 会导致中文路径乱码, 因为clink.get_cwd() 和 clink.prompt.value 获取的编码不同, 之后再处理的clink.prompt.value已经不是原始值了  
-- 参考 https://github.com/cmderdev/cmder/pull/1070 https://github.com/cmderdev/cmder/issues/1054 https://github.com/cmderdev/cmder/wiki/Customization https://github.com/cmderdev/cmder/issues/338#issuecomment-236418658  
-- 可重新注册, 替换原始的λ字符串, 通过翻转替换获取最后一个λ, 避免替换掉路径中的λ文件名  

-- new prompt filter function  
local function my_prompt_filter()  
    local oldLambda = "λ"  
    local newLambda = "$"  
    local oldPromptReverse = string.reverse(clink.prompt.value)  
    local promptReverse = string.gsub(oldPromptReverse,string.reverse(oldLambda),string.reverse(newLambda),1)  
    clink.prompt.value = string.reverse(promptReverse)  
end  
-- register_filter the custom function  
...
#### cmder 整合 intellij idea

terminal `File | Settings | Tools | Terminal` 下 `shell path` 输入其他terminal的路径, 如 cmder `"cmd.exe" /k "C:\LS\cmder\vendor\init.bat"`  

#### cmder 注册右键打开

`cmder /register all`

单窗口打开`HKEY_CLASSES_ROOT\Directory\Background\shell\Cmder\command`中间添加`/single`参数  

`user_aliases.cmd`修改别名
...
方式一: 通过设置jmx的打开方式为jmeter的bin目录下的jmeter-t.cmd  

方式二: 如何方式一不生效, 可通过以下命令设置  
