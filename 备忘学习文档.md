...
=======  
    - [异步函数执行顺序](#%E5%BC%82%E6%AD%A5%E5%87%BD%E6%95%B0%E6%89%A7%E8%A1%8C%E9%A1%BA%E5%BA%8F)  
    - [npm install](#npm-install)  
>>>>>>> ac51d6e1a7dd38ba961123d32db3ff7c638a1ce7  
  - [mongo](#mongo)  
    - [CAP理论](#cap%E7%90%86%E8%AE%BA)  
    - [安装步骤install](#%E5%AE%89%E8%A3%85%E6%AD%A5%E9%AA%A4install)  
    - [备份与恢复 backup and restore](#%E5%A4%87%E4%BB%BD%E4%B8%8E%E6%81%A2%E5%A4%8D-backup-and-restore)  
...
2. 解压zip文件, 修改`my-default.ini`, 改为`my.ini`  
...
3. 进入bin文件夹, 安装mysql服务 `mysqld --install servername --defaults-file='path\my.ini'`, 默认服务名为MySQL, 默认文件为`my.ini`  

4. `net start servername` 根据创建的服务名, 启动服务  

5. `mysql -uroot -p` 登录, `set password=password('123456');` 修改密码  

6. 如有错误, 像1067, 请打开事件查看器, 查看失败的具体原因 `eventvwr.msc`, 然后解决. `sc delete servername` 删除服务  

7. 如何解决"指定的服务已被标记为删除"错误？ `taskkill /F /IM mmc.exe`  

8. 如果用相同的程序, 别忘了修改 basedir为程序目录  
