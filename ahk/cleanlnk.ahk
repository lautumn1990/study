...
; 获取lnk路径
#f2:: ;<-- win + f2 获取lnk路径
    ScrollBox(listFiles("C:\Users\lautumn\Desktop\桌面快捷方式"),"P f{s12 cBlack, Lucida Console} x100 y100 h600 w1200")
    ...
    ;-------------------------------------------------------------------------------
        ...
        shortLen := 10
        Loop, Files, %Folder%\*.*, R
            if (A_LoopFileExt = "lnk" or A_LoopFileExt = "bat" ) {
                ; replace lnk-files with paths to linked target
                FileGetShortCut, %A_LoopFileLongPath%, linkedTarget

                curfilename:=StrReplace(A_LoopFileLongPath, Folder . "\", "")
                if (linkedTarget = ""){
                    linkedTarget := A_LoopFileFullPath
                }
                fullName := curfilename . getSpace(totalLen - StrLen(curfilename)) 
                List .= fullName "`t" linkedTarget "`r`n"
            } else if (A_LoopFileName = "desktop.ini"){
                Continue
            } else
...
; 删除无效的link文件
#f3:: ;<-- win + f3 删除无效的link文件
    ScrollBox(cleaningLinkFile("C:\Users\lautumn\Desktop\桌面快捷方式"),"P f{s12 cBlack, Lucida Console} x100 y100 h600 w1200")
    ;-------------------------------------------------------------------------------
    cleaningLinkFile(Folder) { ; delete invalid link file
    ;-------------------------------------------------------------------------------
        List := ""
        Loop, Files, %Folder%\*.*, R
